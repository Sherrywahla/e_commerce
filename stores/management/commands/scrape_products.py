...
class Command(BaseCommand):
    ...
    def extract_product_details_from_link(self, url):
        try:
            ...
            specifications_section = soup.find("section", class_="card aim -mtm -fs16")
            specifications = {}
        ...
        try:
            ...
            for product in soup.find_all("article", class_="prd"):
                ...
                if a_tag:
                    ...
                    product_data["brand"] = a_tag.get('data-ga4-item_brand', 'N/A')
                    product_data["category"] = a_tag.get('data-ga4-item_category', 'N/A')
                    href = a_tag.get('href')
                    if href:
                        product_details = self.extract_product_details_from_link(href)
                        ...
                        product_data["images"] = [self.download_image(i, save_dir) for i in images_urls if self.download_image(i, save_dir)]
                    else:
                ...
                product_data["reviews"] = review_tag.text.strip() if review_tag else "No reviews"
    ...
    def scrape_all_products(self, base_url, save_dir):
        ...
        while next_page_url:
            ...
            products, next_page_url = self.scrape_products_from_page(next_page_url, save_dir)
    ...
    def handle(self, *args, **kwargs):
        ...
        all_product_data = self.scrape_all_products(start_url, image_save_directory)

        output_file = "products.json"
        with open(output_file, "w", encoding="utf-8") as file:
            json.dump(all_product_data, file, ensure_ascii=False, indent=4)

        self.stdout.write(self.style.SUCCESS(f"Scraped data saved to {output_file}"))
        self.stdout.write(self.style.SUCCESS(f"Images URLs saved (no image downloading) to {image_save_directory}"))
